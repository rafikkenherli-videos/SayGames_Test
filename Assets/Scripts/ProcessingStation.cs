...
public class ProcessingStation : MonoBehaviour
{
    ...
    private bool _transferBusy; // guard: не запускать второй перенос, пока первый не кончился
    ...
    private IEnumerator TransferFromBackpack()
    {
        ...
        // Пока есть место на входе и есть предметы у игрока — переносим по одному
        while (_inputItems.Count < inputPlatform.Capacity && playerBackpack.Count > 0)
        {
            ...
            // --- ДЕКАПЛИНГ ОТ ЯКОРЯ ---
            // Сохраним мировую позу и временно отцепим от якоря, чтобы движение игрока
            // не влияло на позицию во время scale-out
